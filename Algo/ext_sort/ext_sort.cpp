...
int INPUT_OFFSET = 8;
int OUTPUT_OFFSET = 8;
int ACCEPTABLE_SIZE = 12500; // in bytes
ifstream input;
...
void copy_from_to(fstream* from, fstream* to, int offset, int size, qword* buffer, int buffer_size) {
...
void ext_merge_sort(int offset, int size) {
	...
	if (size <= ACCEPTABLE_SIZE) {
		...
		write_to_output(&output, A, offset, size);
	} else {
		...
		copy_from_to(&output, &buffer, real_offset, real_size, 
			BUFFER, 2 * ACCEPTABLE_SIZE);
	}
}

int main() {
	generate_input_bin(1250000);
